...
def build_leaflet_map_html(frame: pd.DataFrame) -> str:
    ...
    # allow_nan=True: 部分 row 的 lat/long 是 NaN, JSON 规范不允许 NaN 字面量,
    # 所以要么过滤空行, 要么开 allow_nan=True。NaN 在 JS 里也会变成 null/None,
    # map 上对应的 marker 不会出现, 等同于过滤。
    payload = json.dumps(points, ensure_ascii=False, allow_nan=True, default=str)
